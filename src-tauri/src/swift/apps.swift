        ...
        NSWorkspace.shared.openApplication(at: url, configuration: configuration) { app, error in
            ...
            guard app != nil else {
                let genericError = NSError(
                    domain: "AppHandler", code: -2,
                    userInfo: [NSLocalizedDescriptionKey: "Application failed to launch"])
...
public func getAppIconSwift(path: UnsafePointer<CChar>?) -> UnsafeMutablePointer<CChar>? {
    guard let path = path,
        let pathString = String(cString: path, encoding: .utf8)
    else {
        return nil
    }
